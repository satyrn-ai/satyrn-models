...
- `source_class` for these entries is `"third-party"` (hyphen, not
  ...
  already uses in its fixtures).
- Every new seed's literal text must be copied byte-for-byte from the
  pinned commit — no paraphrasing, no "close enough" quoting.
...
Expected: PASS (unchanged — these tests exercise `SourceRecord`/`sources.py`
directly, not the committed manifest, except the network-marked
`TestRealManifest` class, which is skipped by default and untouched by this
task).
...
- Test: `tests/authoring/test_seed_artifacts.py` (existing file, counts
  updated in Task 4 — this task only adds a scoped new test for the
  third-party occurrences specifically, to keep failures legible)
